...
DatabaseManager::~DatabaseManager() { sqlite3_close(database); }
...
void DatabaseManager::insert_message(std::string json_msg) {
  ...
  if (!json_msg_parse.contains("type") || json_msg_parse["type"] != "message")
    return;
...
std::string DatabaseManager::get_history_json(int k) {
  const char *get_sql = "SELECT nickname, text FROM (SELECT * FROM messages "
