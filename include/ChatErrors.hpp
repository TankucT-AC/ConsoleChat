...
namespace chat_errors {
    ...
    inline const std::string PREPARE_INSERT_ERROR = "Не удалось подготовить оператор INSERT";
    inline const std::string EXECUTE_INSERT_ERROR = "Не удалось выполнить операцию INSERT";
    inline const std::string RESTORE_HISTORY_ERROR = "Не удалось восстановить историю чата";
    ...
    public:
        ...
        explicit DatabaseError(const std::string& msg, int code = -1) 
            : ChatException("DB Error (" + std::to_string(code) + "): " + msg), error_msg(msg) {}
    };
