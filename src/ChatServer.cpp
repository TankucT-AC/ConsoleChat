...
ChatServer::ChatServer() {
  ...
  chat_server.set_open_handler([this](connection_hdl hdl) {
    ...
    std::cout << "Подключился новый пользователь. Всего: " << user_list.size()
              << std::endl;
    websocketpp::lib::error_code ec;
    chat_server.send(hdl, db.get_history_json(my_config::HISTORY_LIMIT),
  ...
  chat_server.set_close_handler([this](connection_hdl hdl) {
    std::lock_guard<std::mutex> lock(server_mutex);
    user_list.erase(hdl);
    std::cout << "Пользователь отключился. Всего: " << user_list.size()
              << std::endl;
  });

  chat_server.set_message_handler(
